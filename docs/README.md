...
| [nursery-navigation.md](nursery-navigation.md) | Embodied phase: adds spatial + motor centers (four centers) and a *fetch-the-named-object* task. A reward-modulated three-factor rule learns to navigate; cross-modal recall then steers it to the right object (fetch 0.91 vs 0.27 lesioned). Reaches sensorimotor + cross-modal milestones. Run with `--nav-seeds N`. |
| [nursery-permanence.md](nursery-permanence.md) | Object-permanence phase: adds a fifth center (episodic memory). The agent names + remembers objects, they are hidden, then it *searches* for a named one from memory (search 1.00 vs 0.26 lesioned ≈ 1/4). Reaches the object-permanence milestone. Run with `--perm-seeds N`. |
| [nursery-consolidation.md](nursery-consolidation.md) | Sleep/replay consolidation phase: adds a sixth center (semantic memory). Noisy episodic observations are replayed during "sleep" into a durable rule that denoises (0.97 vs 0.66) and abstracts to unseen combos (0.79 vs 0.54 episodic). Plan §6.2 sleep + §5.2 semantic memory. Run with `--consol-seeds N`. |
| [nursery-structural-consolidation.md](nursery-structural-consolidation.md) | Folds consolidation into the slow structural clock: replay accumulates evidence, then developmental pruning keeps the most-evidenced synapses. Evidence-pruned 16 synapses beat the same-budget random-sparse readout (0.91/0.76 vs 0.75/0.64) and approximate the dense readout's ~58 effective wires (0.94/0.81) at a small ~0.04 cost. Plan §6.3, RQ4. |
